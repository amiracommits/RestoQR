...
export default async function CocinaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  //3. Obtener data de restaurante para validar permisos
  ...
  if (perfil.rol !== 'super_admin' && perfil.restaurante_id !== restaurante.id) {
    console.error(`Intento de acceso no autorizado: Usuario ${user.email} trató de entrar a ${slug}`);
