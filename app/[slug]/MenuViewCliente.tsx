...
interface Props {
  ...
  menu: Categoria[];
  mesaId?: string;
  esPedidoAdicional: boolean;
  estadoMesa: string;
}

interface PlatoCompuesto extends Item {
  es_plato_compuesto: true;
  cant_complementos: number; // <-- obligatorio
}

export default function MenuViewCliente({
  restaurante,
  menu,
  mesaId,
  esPedidoAdicional,
  ...
  const [platoActual, setPlatoActual] = useState<Item | null>(null);
  ...
  const listaComplementos = useMemo(() => {
    return menu
      ...
      .filter((item) => item.es_complemento);
  }, [menu]);
  ...
  const agregarAlPedido = (item: Item, notas?: string) => {
  ...
  const handleIntentoAgregar = (item: Item) => {
  ...
  const getProductImage = (
    ...
    if (url) return url;
    if (unsplashId)
      return `https://images.unsplash.com/photo-${unsplashId}?auto=format&fit=crop&q=80&w=400`;
  ...
  const crearPedidoFinal = async () => {
    if (!mesaId) {
      alert(
        "No se detectó el número de mesa. Por favor, escanea el QR de nuevo.",
      );
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-[#1E389E] flex flex-col items-center justify-center p-8 text-center text-white">
        ...
        <p className="text-blue-100 mb-10">
          Tu orden ya está en la cocina. El personal llegará pronto a tu mesa.
        </p>
  ...
  return (
    <main className="min-h-screen bg-slate-50 flex flex-col relative pb-24">
      ...
      {/* 🚀 Nuevo Banner Basado en Estado de Mesa */}
      {mesaId && (
        <div
          className={`border-b p-3 flex items-center gap-3 transition-colors duration-500 ${
            estadoMesa === "ocupada"
          ...
          <p className="text-[11px] font-bold uppercase tracking-tight">
            {estadoMesa === "ocupada" ? (
              <span className="text-amber-800">
                Tienes una cuenta abierta. <br />
                <span className="text-amber-600 font-medium normal-case">
                  Tus nuevos pedidos se sumarán al total de tu mesa.
                </span>
              </span>
            ) : (
              <span className="text-emerald-800">
                ...
                <span className="text-emerald-600 font-medium normal-case">
                  Estás iniciando una nueva orden.
                </span>
              </span>
            )}
          </p>
      ...
      <header
        className={`bg-[#1E389E] sticky ${mesaId ? "top-[20px]" : "top-0"} z-[60] shadow-md p-4 flex justify-between items-center h-[65px] border-b border-blue-800/30`}
      >
      ...
      <nav
        className={`bg-white sticky ${mesaId ? "top-[85px]" : "top-[65px]"} z-50 border-b border-slate-100 shadow-inner`}
      >
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {category.items.map((item) => (
                <div
                  key={item.id}
                  className="bg-white rounded-3xl p-4 shadow-sm border border-slate-100 flex gap-4 transition-all active:scale-[0.97]"
                >
          ...
          <div className="relative bg-white w-full max-w-lg rounded-t-[40px] sm:rounded-[40px] shadow-2xl overflow-hidden animate-slide-up flex flex-col max-h-[90vh]">
            <div className="p-8 border-b border-slate-100">
              <div className="flex justify-between items-start mb-2">
                ...
                <button
                  onClick={() => setModalAbierto(false)}
                  className="text-slate-300 hover:text-slate-500 text-2xl"
                >
                  ✕
                </button>
              </div>
              <p className="text-slate-500 font-bold">
                Selecciona{" "}
                <span className="text-[#1E389E] underline">
                  {platoActual.cant_complementos} acompañamientos
                </span>{" "}
                para continuar.
              </p>
            </div>
              ...
              {listaComplementos.length > 0 ? (
                listaComplementos.map((comp) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (estaSeleccionado) {
                          ...
                          (platoActual.cant_complementos ?? 0)
                        ) {
                    ...
                    >
                      <span
                        className={`font-bold text-lg ${estaSeleccionado ? "text-orange-700" : "text-slate-700"}`}
                      >
                        {comp.nombre}
                      </span>
                      <div
            ...
            <div className="p-8 bg-slate-50 border-t border-slate-100">
              ...
              >
                {seleccionados.length === platoActual.cant_complementos
                  ...
                  : `Faltan {(platoActual.cant_complementos ?? 0) - seleccionados.length}`}
              </button>
            </div>
          </div>
        </div>
            ...
            <div className="p-8 border-b border-slate-100 flex justify-between items-center">
              <div>
                ...
                <p className="text-slate-500 text-sm font-medium italic">
                  Ej: Sin cebolla, Término medio, etc.
                </p>
              </div>
              <button
              ...
              {itemsAgrupados.map((item) => (
                ...
                >
                  <div className="flex justify-between mb-3">
                    ...
                    <span className="text-[10px] font-black bg-slate-200 px-2 py-1 rounded-full uppercase text-slate-500">
                      Opcional
                    </span>
                  </div>
            ...
            <div className="p-8 bg-white border-t border-slate-100">
              <button
                ...
                onClick={async () => {
                  ...
                  if (!mesaId) {
                    alert(
                      "Error: No se detectó el número de mesa. Por favor, escanea el QR de nuevo.",
                    );
                    return;
                  }
                  setEnviando(true);
